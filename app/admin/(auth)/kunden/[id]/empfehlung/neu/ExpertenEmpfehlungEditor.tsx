...
export default function ExpertenEmpfehlungEditor({ customerId, initial, initialEinleitung }: Props) {
  ...
  function hinzufuegen() {
    ...
    if (!a) return;
    const research = RESEARCH[a.slug as AnwendungSlug];
    setEintraege((prev) => [
      ...prev,
      {
        slug: a.slug,
        haeufigkeitText: research?.sessions ?? '',
        begruendung: '',
      },
    ]);
    setAddSlug('');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <label className="block text-xs font-medium text-gray-600 mb-1">
          ...
          <span className="ml-1 text-gray-400 font-normal">(personalisiert)</span>
        </label>
        <textarea
          value={einleitung}
      ...
      {eintraege.map((eintrag, index) => {
        ...
        return (
          ...
          >
            ...
            {/* Wirkmechanismus (read-only, aus research.ts) */}
            {research && (
      ...
      <div className="flex justify-end gap-2">
        ...
        >
          {saving ? 'Speichert…' : 'Expertenempfehlung speichern'}
        </button>
      </div>
    </div>
  );
